# xiuxian_helper

一个运行在 Telegram 群组/话题里的“修仙文字游戏”自动化脚本（基于 Telethon），采用插件化结构：玩家（你）在群里操作，脚本负责按规则定时/按回包触发地发送指令，并将“我发出的指令/系统回应”以更干净的日志输出。

## 功能概览

### 1) 自动闭关（默认启用）
- 发送闭关指令（默认：`.闭关修炼`，可配置）。
- 解析回包中的冷却时间（“打坐调息 N 分钟”“灵气尚未平复…请在 N秒后再试”），到点自动重试（包含 `0秒` 场景）。

### 2) 自动种植（小药园）
插件：`garden`
- 定期发送 `.小药园` 获取状态，并解析：
  - 干旱/虫害/杂草：分别执行 `.浇水` / `.除虫` / `.除草`
  - 成熟：执行 `.采药`，随后（若有空闲灵田）执行 `.播种 <种子名>`
  - 空闲：执行 `.播种 <种子名>`（一键补满空闲灵田）
- 轮询会根据“最短剩余时间”自动提前检查，避免错过成熟时间。

### 3) 星宫（观星台 + 周天星斗大阵 + 每日问安 + 深度闭关联动）
插件：`xinggong`
- 观星台
  - 定期发送 `.观星台`，解析引星盘状态
  - 有异常：`.安抚星辰`
  - 可收集：`.收集精华`
  - 有空闲：`.牵引星辰 <星辰>`（不指定盘号，由系统自动填补空白）
  - 同样根据“最短剩余时间”自动提前复查
- 观星劫持（可选开关）
  - 监听群内所有 `【星盘显化】` 消息
  - 若“下一次天道演化将是”命中配置事件（默认：`星辰异象,地磁暴动`），先记录本 3 小时窗口机会
  - 到结算前按 `XINGGONG_GUANXING_PREVIEW_ADVANCE_SECONDS` 再发送 `.观星`（默认 180 秒）
  - 当系统回给你自己的 `【星盘显化】` 后，会在结算前按 `XINGGONG_GUANXING_SHIFT_ADVANCE_SECONDS` reply 该消息发送 `.改换星移 @目标用户名`
  - 为避免把上一窗口结算前发出的 `.观星` 迟到回包误判为新窗口事件，脚本会忽略每个新窗口最开始 5 分钟内的外部 `【星盘显化】`
  - 为了保证卡秒，脚本会在临近改换星移前的关键窗口内暂缓其他非关键发送
- 周天星斗大阵
  - 按配置时间开始 `.启阵`，若未成功则按配置间隔重试直到成功
  - 监听“阵成通知”（包含消息编辑场景），成功后自动进入下一轮排程
  - 若收到冷却提示（如“请在X小时X分钟X秒后再次启阵”），会更新下一次可尝试时间，避免在冷却期内刷屏
  - 自动助阵：检测到他人启阵邀请后自动发送 `.助阵`（无需 reply）
  - 可选深度闭关联动：启阵成功后自动发送 `.查看闭关`，若已在深度闭关则 `.强行出关 -> .深度闭关`，若未在深度闭关则直接 `.深度闭关`；并在成功后第 5 小时再刷新一次
  - 开启该联动后，普通自动闭关 `.闭关修炼` 会自动暂停，避免并发冲突
- 每日问安
  - 按 `XINGGONG_WENAN_INTERVAL_SECONDS` 发送 `.每日问安`（默认 12 小时）
  - 可用 `ENABLE_XINGGONG_WENAN=0` 单独关闭，不影响观星台/启阵/助阵

### 4) 元婴期（日常探寻 + 元婴出窍）
插件：`yuanying`
- 按 `YUANYING_LIEFENG_INTERVAL_SECONDS` 尝试 `.探寻裂缝`（默认 12 小时）
- 若回包提示“请在X小时X分钟X秒后再行探寻”，会按剩余时间同步下一次尝试
- `【遭遇风暴】` / `【不敌败退】` 会快速重试；`【元婴遁逃·虚弱】` 会等待虚弱结束后再尝试
- 元婴出窍改为状态驱动：启动或纠偏时先查 `.元婴状态`
- 若状态为 `状态:元神出窍 归来倒计时:X小时X分钟X秒`，会按剩余时间等待；到点后自动发送 `归来` 触发结算
- 收到包含 `元神归窍总结` 的回包后，会立即再次 `.元婴出窍`

### 5) 每日闯塔
插件：`chuangta`
- 每天固定时间发送 `.闯塔`（默认 `14:15`）
- 若启用了元婴插件，则会先查 `.元婴状态`
  - `状态:窍中温养`：立即闯塔
  - `状态:元神出窍`：等待 `元神归窍总结` 后补闯塔
- 若未启用元婴插件，或角色尚未达到元婴期，则直接按时间执行 `.闯塔`
- 收到闯塔相关结算回包后，标记当天已完成，避免重复发送

### 6) 宗门日常（点卯 + 传功）
插件：`zongmen`
- 每天定时 `.宗门点卯`
- 每天定时传功 3 次：先发一条“心得”（可配置），再回复该消息发送 `.宗门传功`

## 日志与限流策略
- 默认 `LOG_LEVEL=INFO` 时，只输出两类关键信息：
  - `>>` 我方发送的指令
  - `<<` 系统回包（仅“高置信相关”的消息）
- 发送限流：
  - `GLOBAL_SEND_MIN_INTERVAL_SECONDS`：全局最小发送间隔；非会员号建议设为 `10`
  - `GLOBAL_SENDS_PER_MINUTE`（全局）
  - `PLUGIN_SENDS_PER_MINUTE`（每插件）
  - 所有发送现在都会走全局串行队列；若 Telegram 要求等待或短时发送失败，会自动重试，直到发出为止。

## 持久化恢复
- 脚本现在会把关键运行状态写入本地 sqlite：默认文件 `xiuxian_state.sqlite3`
- 可通过 `STATE_DB_PATH` 修改路径
- 重启后会尽量恢复这些关键节点：
  - 自动闭关下一次触发时间
  - 小药园 / 观星台的下一次轮询时间
  - 星宫启阵冷却、深度闭关联动、观星劫持窗口
  - 元婴探寻裂缝 / 元婴出窍冷却
  - 每日闯塔当天进度
  - 宗门点卯 / 传功当天进度
- 设计目标是“恢复关键时间点与进度”，避免因进程重启把主要自动化节奏打断。

## 快速开始

### 1) 安装依赖
- Python >= 3.10
- 安装 Telethon：
```bash
python3 -m pip install telethon
```

（可选）使用虚拟环境：
```bash
python3 -m venv venv
source venv/bin/activate
python3 -m pip install telethon
```

### 2) 配置
复制示例配置并填写必要参数：
```bash
cp .env.example .env
```

你至少需要设置：
- `TG_API_ID` / `TG_API_HASH`（从 https://my.telegram.org 获取）
- `GAME_CHAT_ID`（群组 id）
- `TOPIC_ID`（如果群是论坛话题：话题 root message id）
- `MY_NAME`（你在游戏里的名字，用于过滤专属回包）

然后按需打开插件：
- `ENABLE_BIGUAN=1`
- `ENABLE_GARDEN=1`
- `ENABLE_XINGGONG=1`
- 若只想关闭星宫问安：`ENABLE_XINGGONG_WENAN=0`
- 若想启阵后联动深度闭关：`ENABLE_XINGGONG_DEEP_BIGUAN=1`
- 若想开启观星劫持：`ENABLE_XINGGONG_GUANXING=1`
- `ENABLE_YUANYING=1`
- `ENABLE_CHUANGTA=1`
- `ENABLE_ZONGMEN=1`

建议第一次先用 `DRY_RUN=1` 观察日志确认无误后再改为 `0`。

### 3) 运行
从项目根目录启动：
```bash
python3 xiuxian.py
```

首次运行会进行 Telegram 登录并生成本地会话文件（`.session`）。

## 作为 systemd 服务运行（可选）
下面是 user service 示例（更推荐，不需要 root）：

1) 创建：`~/.config/systemd/user/xiuxian-helper.service`
```ini
[Unit]
Description=Xiuxian Helper
After=network.target

[Service]
Type=simple
WorkingDirectory=/path/to/xiuxian
ExecStart=/path/to/xiuxian/venv/bin/python /path/to/xiuxian/xiuxian.py
Restart=always
RestartSec=5

[Install]
WantedBy=default.target
```

2) 启动：
```bash
systemctl --user daemon-reload
systemctl --user enable --now xiuxian-helper.service
journalctl --user -u xiuxian-helper.service -f
```

如果你不使用 venv，把 `ExecStart` 的 python 路径改成 `/usr/bin/python3` 即可。

---

如需扩展新玩法，推荐新增一个 `xiuxian_bot/plugins/<feature>.py` 插件，并实现 `on_message()`（观察回包 -> 产出 SendAction），配合 `Scheduler` 做定时任务。
